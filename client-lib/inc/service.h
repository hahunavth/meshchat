...
extern int _get_user_search(const char *uname, uint32_t *_res);
...
extern int _get_conv_members(const uint32_t conv_id, uint32_t *_res);
...
extern int _get_chat_list(const int limit, const int offset, uint32_t *_res);
